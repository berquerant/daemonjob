...
func daemonJobSamplesBeforeAddNode() utils.Testcase {
	return utils.Testcase{
		Name:         "DaemonJob/SamplesBeforeAddNode",
		...
		BeforeAddNode: func(a *utils.TestcaseArgs) {
			...
			It("Should successfully handle sample manifest", func() {
				...
				_, err := utils.Run(utils.KubectlCmd("-n", namespace, "delete", "daemonjob", resourceName))
				Expect(err).To(Succeed())

				By("ServiceAccount should be deleted")
				utils.EventuallyNotFoundByLabelSelector(namespace, "sa",
					controller.DaemonJobLabelDaemonJobName, resourceName,
				...
				By("Broadcast Job should be deleted")
...
func daemonJobSamplesAfterAddNode() utils.Testcase {
	return utils.Testcase{
		Name:         "DaemonJob/SamplesAfterAddNode",
		KustomizeDir: "samples",
