...
func daemonCronJobSetNodeSelectorBeforeAddNode() utils.Testcase {
	return utils.Testcase{
		Name:         "DaemonCronJobSet/NodeSelectorBeforeAddNode",
		...
		BeforeAddNode: func(a *utils.TestcaseArgs) {
			...
			It("Should successfully handle node_selector manifest", func() {
				...
				By("CronJobs should not be created")
				Consistently(func(g Gomega) {
					...
					g.Expect(xs).To(BeEmpty())
				}).To(Succeed())

				By("The resource state should be changed into Available")
				utils.EventuallyDaemonCronJobSetState(namespace, resourceName, "Available", 0, 0).To(Succeed())
			})
		},
		AfterAddNode: func(a *utils.TestcaseArgs) {
			...
			It("Should successfully handle node_selector manifest", func() {
				...
				utils.EventuallyDaemonCronJobSetCronJobNodes(namespace, resourceName, utils.NodeName(1)).To(Succeed())
		...
		AfterRemoveNode: func(a *utils.TestcaseArgs) {
			...
			It("Should successfully handle node_selector manifest", func() {
				...
				By("CronJobs should not be remaining")
				utils.EventuallyNotFoundByLabelSelector(namespace, "cronjob",
					controller.DaemonJobLabelDaemonCronJobSetName, resourceName,
					controller.DaemonJobLabelRole, controller.DaemonJobLabelRoleWorker,
				).To(Succeed())
